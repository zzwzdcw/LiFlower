...
import { getM19Cache } from '@/utils/cacheManager'
...
onMounted(() => {
  ...
  if (cache && cache.equipmentList && cache.equipmentList.length > 0) {
    ...
    console.log('[M19] 从缓存恢复装备:', equipmentList.value)
  } else {
    // 缓存为空时重置为初始状态
    equipmentList.value = []
    equipmentIdCounter = 0
    console.log('[M19] 缓存为空，重置装备')
  }
})
...
onMounted(() => {
  if (window.liflowerCache) {
    window.liflowerCache.registerM19({
      equipmentList
    })
    console.log('[M19] 已注册到缓存管理器')
  } else {
    console.warn('[M19] window.liflowerCache 未定义')
  }
})
</script>
