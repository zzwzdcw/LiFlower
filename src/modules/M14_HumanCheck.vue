...
import { getM14Cache } from '@/utils/cacheManager'
...
onMounted(() => {
  ...
  if (cache.combatToolBonuses && Array.isArray(cache.combatToolBonuses)) {
    ...
    console.log('[M14] 从缓存恢复战斗数据工具加值:', cache.combatToolBonuses)
  }
  ...
  if (cache.checkToolBonuses && Array.isArray(cache.checkToolBonuses)) {
    ...
    console.log('[M14] 从缓存恢复检定数据工具加值:', cache.checkToolBonuses)
  }
})
...
onMounted(() => {
  if (window.liflowerCache) {
    window.liflowerCache.registerM14({
    ...
    console.log('[M14] 已注册到缓存管理器')
  } else {
    console.warn('[M14] window.liflowerCache 未定义')
  }
})
</script>
